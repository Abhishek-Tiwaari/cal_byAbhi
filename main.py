...
def all_clear():   # for all clear button
    textField.delete(0, END)
...
def click_btn_function(event):
    ...
    if text == '=':
        try:
            ...
            print("Error..", e)
            showerror("Error", e) # tkinter mssgbox method (invalid entry)
        return
...
headingLabel.pack(side=TOP, pady=10)
        ...
        # heading label
heading = Label(window, text='Structured by Abhishek', font=font)
...
textField.pack(side=TOP, pady=10, fill=X, padx=10)
         ...
         # for buttons create a frame which is not visible
...
textField.bind('<Return>', enterClick) # Binding to the keyboard
